...
if [ ${#FORMAT_ARGS[@]} -gt 0 ]; then
    ...
    if [ -n "$format_options" ]; then
        ...
        for _fo in "${_FOPTS[@]}"; do
            [ -n "$_fo" ] || continue
            [[ "$_fo" == *=* ]] || { echo "ERROR: format_options entry must be key=value: $_fo" >&2; exit 1; }
            _fkey="${_fo%%=*}"
            _fval="${_fo#*=}"
            [[ "$_fkey" =~ ^[A-Za-z0-9][A-Za-z0-9_-]*$ ]] || { echo "ERROR: invalid format_options key: $_fo" >&2; exit 1; }
            [[ "$_fval" =~ ^[A-Za-z0-9_./%+-]*$ ]] || { echo "ERROR: invalid format_options value: $_fo" >&2; exit 1; }
            case "$_fkey" in
