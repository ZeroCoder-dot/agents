...
// CustomFuseDriver is the CSI driver name of the generic FUSE plugin
// (alibaba-cloud-csi-driver customfuse). It MUST match both the directory
// under CsiSocketDir created by the csi-sidecar and the driver name used by
// the control-plane storage registration.
const CustomFuseDriver = "customfuseplugin.csi.alibabacloud.com"
...
// Validate checks the fields required to route a publish request to the
// sidecar. Deep option validation (source, fuseType whitelist, credential
// separation) is done by the control-plane CustomFuseMountProvider at claim
// time; the sidecar CSI node server repeats the environment-key checks
// (Secret keys and mount flags) as defense in depth, and the FUSE entrypoint
// re-validates shell safety of the values it interpolates.
...
// RunNodePublishVolume (see csi_runner.go).
