...
func parseOptions(req *csi.NodePublishVolumeRequest) (*fuseOptions, error) {
	...
	for k, v := range req.GetVolumeContext() {
		...
		if existing, ok := seen[key]; ok {
			if existing != value {
				return nil, fmt.Errorf("conflicting values for %q: %q and %q", key, customfusevalidate.MaskOptionValues(existing), customfusevalidate.MaskOptionValues(value))
			}
			continue
		}
		seen[key] = value
		...
		case "capacity":
			if !customfusevalidate.CapacityPattern.MatchString(value) {
				return nil, fmt.Errorf("invalid capacity %q: must be a plain integer or one of Ti/TiB, Gi/GiB, Mi/MiB, Ki/KiB units (e.g. 100, 100Gi)", customfusevalidate.MaskOptionValues(value))
			}
			opts.Capacity = value
		case "storagetype":
	...
	// inside the sandbox without passing through the provider. The same
	// shared character set the provider uses is enforced here; otherOpts
	// and capacity carry option-style values and get the metachar check.
	for _, field := range []struct{ name, value string }{
		...
		if field.value != "" && !customfusevalidate.SafeForwardPattern.MatchString(field.value) {
			return nil, fmt.Errorf("%s contains invalid characters: %q", field.name, customfusevalidate.MaskOptionValues(field.value))
		}
	}
	for _, field := range []struct{ name, value string }{
