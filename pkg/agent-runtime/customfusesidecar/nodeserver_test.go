	...
	tests := []struct {
	...
	}{
		{
			...
			assertReq: func(t *testing.T, got *proxyMountRequest) {
				...
				assert.Equal(t, "/run/csi/mount-root/customfuse/vol-1", got.Target)
...
func TestNodePublishVolumeSerializesConcurrentRequests(t *testing.T) {
	...
	ns := NewNodeServer("/var/run/csi/mounter.sock")
	req := &csi.NodePublishVolumeRequest{VolumeId: "vol-lock", TargetPath: "/run/csi/mount-root/customfuse/vol-1"}
...
func TestNodeGetCapabilities(t *testing.T) {
	...
	assert.Equal(t, csi.NodeServiceCapability_RPC_STAGE_UNSTAGE_VOLUME, resp.Capabilities[0].GetRpc().Type)
}
